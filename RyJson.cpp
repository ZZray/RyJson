...
private:
    ...
    Json getBool()
    {
        if (_str.compare(_cursor, 4, "true")) {
            _cursor += 4;
            return true;
        }
        if (_str.compare(_cursor, 5, "false")) {
            _cursor += 5;
            return false;
        }
...
Json Json::parseFile(std::string_view filename, int* errorCode, std::function<void(int)> progressCallback)
{
    try {
        ...
        return parse(std::string{std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>()}, errorCode, progressCallback);
    }
    catch (...) {
    }
    return {};
